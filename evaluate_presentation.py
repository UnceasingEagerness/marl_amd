...
def run_simulation(num_agents_sim, out_filename):
    ...
    # 100m spaced spawns
    starts_x_rl = jnp.array([-800, -700, -900, -800], dtype=jnp.float32)
    starts_y_rl = jnp.array([-700, -800, -800, -900], dtype=jnp.float32)
    angles_rl = jnp.array([jnp.pi/4, jnp.pi/4, jnp.pi/4, jnp.pi/4], dtype=jnp.float32)
    goals_x_rl = jnp.array([800, 700, 900, 800], dtype=jnp.float32)
    goals_y_rl = jnp.array([700, 800, 800, 900], dtype=jnp.float32)
    ...
    ax_map.set_title(f"Swarm Navigation (N=4, Dyn Obs=6)", color='black', fontsize=16)
...
if __name__ == "__main__":
    ...
    run_simulation(10, "visualizations_contributions/presentation_clean.gif")
