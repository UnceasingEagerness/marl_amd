...
class TemporalActorBackbone(nn.Module):
    ...
    def __call__(self, x_flat):
        ...
        LSTM = nn.RNN(nn.LSTMCell(features=128, kernel_init=nn.initializers.lecun_normal(), recurrent_kernel_init=nn.initializers.lecun_normal()), return_carry=True)
...
class FlaxSpatioTemporalAttentionEncoder(nn.Module):
    ...
    def __call__(self, ego_seq, entity_seqs):
        ...
        LSTM_ego = nn.RNN(nn.LSTMCell(self.hidden_dim, kernel_init=nn.initializers.lecun_normal(), recurrent_kernel_init=nn.initializers.lecun_normal()), return_carry=True)
        ...
        LSTM_shared = nn.RNN(nn.LSTMCell(self.hidden_dim, kernel_init=nn.initializers.lecun_normal(), recurrent_kernel_init=nn.initializers.lecun_normal()), return_carry=True)
