...
from algorithms.flax_sac import STAE_ActorBackbone
...
def main():
    ...
    # 1000m map with 400 obstacles creates an ultra-dense field
    env_params = env.default_params.replace(num_agents=num_agents, map_size=1000.0, num_obstacles=400, max_steps=2000)
    ...
    actor = LegacyActor(layout=layout, action_dim=action_dim, action_scale=jnp.ones(action_dim), action_bias=jnp.zeros(action_dim))
